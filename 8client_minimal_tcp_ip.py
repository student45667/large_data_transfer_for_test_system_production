...
try:
	while True:
		...
		response_header_of_dlog = inst.query('GET_LOG?')
		print(f"recieved: {response_header_of_dlog}")
		# FIX: Use read_raw() for binary data (not read())
		...
		with open('dump_log.log', 'ab') as f:  # 'ab' = append BINARY
   		# Convert header string to bytes before writing 
			f.write(response_header_of_dlog.encode() + datalog)
